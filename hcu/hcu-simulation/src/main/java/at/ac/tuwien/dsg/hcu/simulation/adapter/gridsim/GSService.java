...
import at.ac.tuwien.dsg.hcu.common.model.PropertyTypes;
...
public class GSService extends ParallelResource {
    ...
    public GSService(Service service, AllocPolicy scheduler) throws Exception {
        this(service.getTitle(),   
                Util.Integer(service.getProvider().getProperties().getValue(PropertyTypes.PROP_PERFORMANCE, 1.0)), 
                Util.Double(service.getProvider().getProperties().getValue(PropertyTypes.PROP_TIMEZONE, 0)), 
                Util.Double(service.getProvider().getProperties().getValue(PropertyTypes.PROP_COST, 1.0)),
                scheduler);
